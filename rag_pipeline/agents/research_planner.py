...
from .dynamic_search_schema import normalize_research_plan
...
输出 JSON：
{
  ...
  "report_family": "industry_scan_report / market_entry_report / company_due_diligence_report / investment_memo / product_research_report / competitor_analysis_report / policy_impact_report / consumer_market_report / macro_topic_report / supply_chain_report / briefing_note",
  "research_object": "...",
  "chapters": [
    {
      "chapter_id": "ch_01",
...
def _dynamic_seed_plan(query: str, problem_framing: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
...
def _llm_research_plan(query: str, llm_config: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {}
    return _as_dict(response.get("payload"))
...
def run_research_planner_agent(
    *,
    ...
    try:
        problem_framing = run_problem_framing_agent(query=query, llm_config=llm_config)
    except Exception:
        logger.exception("Problem framing failed in research planner", extra={"query": query})
        problem_framing = {}
    llm_plan = normalize_research_plan(_llm_research_plan(query, llm_config), query=query)
    if (llm_plan.get("chapters") or llm_plan.get("dimensions")) and llm_plan.get("search_tasks"):
        ...
        plan = normalize_research_plan(_dynamic_seed_plan(query, problem_framing), query=query)
...
def research_planner_tool(payload: Dict[str, Any]) -> Dict[str, Any]:
    return run_research_planner_agent(query=str(payload.get("query") or ""), llm_config=_as_dict(payload.get("llm_config")))
