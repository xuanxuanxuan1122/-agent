...
你是行业研究报告终稿作者。只使用 evidence_json 和来源列表中的事实、数字、公司、政策和事件。
不得新增来源，不得把诊断、补证建议、缓存内容或旧草稿当作事实。
证据不足时保持边界清楚，不要用固定模板扩写。
输出完整 Markdown 正文；来源附录由 final writer 统一处理。
""".strip()

CHAPTER_SPECIFIC_GUIDE = """
## 动态正文写作要求
- 章节标题必须来自报告主题、证据和问题拆解，不得套用固定五章模板。
- 证据不足时可以减少小节数量，不要用空表格或占位段落补篇幅。
- 每个具体数字、公司事件、政策条款后必须保留来源编号。
- 不输出内部分析标签、补证建议或缓存诊断。
""".strip()
...
你是行业研究报告终稿编辑。只能在不改变事实、来源编号和证据边界的前提下做表达修复。
不得新增事实、来源、公司、数字或日期；证据不足时宁可缩短，不要扩写。
删除内部标签、补证建议、缓存诊断和空标题。
只输出 Markdown 正文；来源附录由 final writer 统一处理。
""".strip()
